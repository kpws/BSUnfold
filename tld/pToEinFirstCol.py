...
writer=csv.reader(open('results/'+name, 'wb'), delimiter='\t')
reader.next()
for row in reader:
...
neutronRateMean=np.array(rate6mean)-np.array(rate7mean)
neutronRateStd=np.sqrt(np.array(rate6std)**2+np.array(rate7std)**2)
pl.errorbar(E, neutronRateMean, yerr=neutronRateStd*nstd, fmt='-',label='Li6F-Li7F $\pm '+str(nstd)+'\sigma$')
pl.legend(loc=2)
